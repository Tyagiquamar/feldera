...
# TEST-NET-1 (RFC 5737) is reserved for documentation and is not routed, so a
# connection to it hangs in TCP connect instead of being refused.  A sync
# pointed here stays in `running` long enough to observe deterministically,
# where a sync against a reachable endpoint can finish before the first status
# request arrives.
BLACKHOLE_ENDPOINT = "http://192.0.2.1:9000"
...
class TestCheckpointSync(SharedTestPipeline):
    ...
    def test_sync_status_in_flight(self):
        ...
        CREATE MATERIALIZED VIEW v0 AS SELECT * FROM t0;
        """
        # The sync cannot finish against an unroutable endpoint, so the window
        # in which it is running is wide enough to observe.  Without that, a
        # sync can start and finish between the 202 and the first status
        # request, and an implementation that never populated `running` would
        ...
        self.pipeline.set_runtime_config(
            build_runtime_config(
                ...
                fault_tolerance_model=FaultToleranceModel.AtLeastOnce,
                storage=Storage(
                    config=storage_cfg(self.pipeline.name, endpoint=BLACKHOLE_ENDPOINT)
                ),
